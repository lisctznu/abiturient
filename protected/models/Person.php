...
class Person extends ActiveRecord {
  ...
  protected function afterConstruct() {
    if (empty($this->PhotoName))
      $this->PhotoName = Yii::app()->params['defaultPersonPhoto'];
    //if (empty($this->KOATUUCodeL1ID)) $this->KOATUUCodeL1ID = 105572;
    //if (empty($this->KOATUUCodeL2ID)) $this->KOATUUCodeL2ID = 105574;
    //if (empty($this->KOATUUCodeL3ID)) $this->KOATUUCodeL3ID = 105576;
    $this->IsResident = 1;
...
//17	Посвідка на постійне проживання в Україні
      if ($val->id_Type == 7) {
        ...
        $model->entrantdoc->DateGet = date(
                "d.m.Y", mktime(0, 0, 0, $val->dateGet['month'] + 1, $val->dateGet['dayOfMonth'], $val->dateGet['year']));
...
//            debug($this->entrantdoc->idDocuments);
    try {
      $client = new EHttpClient(Yii::app()->user->getEdboSearchUrl()
              . Yii::app()->params["personAddURL"], array('maxredirects' => 30, 'timeout' => 30,));
